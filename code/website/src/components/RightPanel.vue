...
import CourseRightSummary from "./course/RightSummary.vue";
...
function progress() {
	return (
		(cpAPI.getCourseAssignments()[1] /
			programAPI.getCurrent().required_cp) *
		100
	).toFixed(0);
}
</script>
<template>
	...
	<div v-if="panelNotVisible()" class="rhs">
					...
					<li>
						<h5>
							<strong
								...
								></i>
								No major selected</strong
							>
						</h5>
					</li>
				</template>
				<template
					v-for="req in planAPI.getCurrent().topLevelReqsSelected"
				>
					<li>
						<h5>
							<i
								class="fa-solid fa-triangle-exclamation"
								v-if="
									cpAPI.getCourseAssignments()[0][req]
										.achieved_cp <
									cpAPI.getCourseAssignments()[0][req]
										.required_cp
								"
							...
							>
							(Max
							{{ cpAPI.getCourseAssignments()[0][req].max_cp }})
						</h5>
						<template v-if="getFullProgram(req).sub_requirements">
							<ul>
								<template
									v-for="subreq in getFullProgram(req)
										.sub_requirements"
								>
									<li>
										<h6>
		...
		<template
			v-if="
				cpAPI.getCourseAssignments()[1] ||
				cpAPI.getCourseAssignments()[1] === 0
			"
		>
			<h5 class="text-center">
				Total points: {{ cpAPI.getCourseAssignments()[1] }}/{{
					programAPI.getCurrent().required_cp
				}}
			</h5>
			<div

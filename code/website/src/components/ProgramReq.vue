...
<template>
	...
	<template v-if="getCurrentProgram()?.program_requirements?.[props.index]">
	...
	>
		...
		<option
			v-for="option in getCurrentProgram()!.program_requirements[
				props.index
			]!.map((req) => getProgramRequirement(req))"
